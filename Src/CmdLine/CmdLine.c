...
int Cmd_Debug(int argc, char *argv[])
{
    ...
    {
    	WasteManager_Debug(TRUE);
        Printf("Open Debug Mode\n");
    }
    else  if (IsEqual(argv[1], "close"))
    {
    	WasteManager_Debug(FALSE);
        Printf("Close Debug Mode\n");
    }
    else  if (IsEqual(argv[1], "set"))
	{
    	if(argv[2] && argv[3])
    	{
    		WasteManager_Test(atoi(argv[2]),atoi(argv[3]));
    	}
	}
    else if (0 == argv[1] || IsEqual(argv[1], "help") ||
             IsEqual(argv[1], "?"))
